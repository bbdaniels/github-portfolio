(function() {
  ...
  if (document.readyState === 'loading') {
    ...
    // DOM already loaded (common with Squarespace, WordPress, etc.)
    init()
  }

  // Also watch for dynamically added github-pinner elements
  if (typeof MutationObserver !== 'undefined') {
    var observer = new MutationObserver(function(mutations) {
      mutations.forEach(function(mutation) {
        mutation.addedNodes.forEach(function(node) {
          if (node.nodeType === 1) { // Element node
            if (node.classList && node.classList.contains('github-pinner') && !node.hasAttribute('data-gp-loaded')) {
              node.setAttribute('data-gp-loaded', 'true')
              loadCSS()
              loadElements(node)
            }
            // Also check children
            var pinners = node.querySelectorAll ? node.querySelectorAll('.github-pinner:not([data-gp-loaded])') : []
            for (var i = 0; i < pinners.length; i++) {
              pinners[i].setAttribute('data-gp-loaded', 'true')
              loadCSS()
              loadElements(pinners[i])
            }
          }
        })
      })
